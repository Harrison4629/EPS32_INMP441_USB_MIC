...
* **Led指示灯(可选)**: Led二极管和适当阻值的电阻
* **轻触按键开关(可选)**: 按压导通式按键开关
* **接线引脚**:

| 硬件模块 | 模块引脚 | ESP32-S3 GPIO | 说明 |
| :--- | :--- | :--- | :--- |
| **INMP441 麦克风** | **SCK** | GPIO 4 | 串行时钟线 |
| | **WS** | GPIO 5 | 字选择/左右声道选择线 |
| | **SD** | GPIO 6 | 串行数据线 |
| | **L/R** | GND | 接地（选择左声道） |
| | **VDD** | 3.3V | 供电 |
| | **GND** | GND | 接地 |
| **轻触按键 (可选)** | **PIN 1** | GPIO 1 | 按键输入脚（底层配置上拉，按下导通接地） |
| | **PIN 2** | GND | 接地端 |
| **LED 状态灯(可选)** | **阳极 (长脚)** | GPIO 2 | 状态控制引脚（通过高低电平控制亮灭） |
| | **阴极 (短脚)** | GND | 接地端（需串联 ~220Ω 限流电阻防止烧灯） |
